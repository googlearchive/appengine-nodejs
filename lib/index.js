...
/**
 * Retrieve the OAuth access token from the GCE metadata.
 * In case of success, the callback will be invoked with as argument an object
 * containing two string-valued properties, "token" and "tokenType".
 *
 * @param {!Object} req node.js request object
 * @param {function(?Error, ?Object)} callback
 */
exports.auth.withAccessToken = function(req, callback) {
  var options = {
    hostname: 'metadata',
  ...
  var tokenRequest = http.request(options, function(tokenResponse) {
    ...
    tokenResponse.on('end', function(e) {
      ...
      if (tokenResponse.statusCode == 200) {
        ...
        callback(null, {token: response.access_token, tokenType: response.token_type});
      } else {
