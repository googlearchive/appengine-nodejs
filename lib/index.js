...
AppEngine.prototype.callApi_ = function(serviceName, methodName, req, proto, callback) {
  if (!this.validateRequest_(req, callback)) {
    return;
  }
 ...
 * invoke the specified callback with an error.
 *
 * @param {Object} req request object
 * @param {!Function} callback callback to be invoked in case of error
 * @return true if the request is valid, false otherwise
 */
AppEngine.prototype.validateRequest_ = function(req, callback) {
  if (!goog.isDefAndNotNull(req)) {
    callback(new Error('invalid request object: ' + req));
    return false;
  }
  if (!goog.isDefAndNotNull(req.appengine)) {
    callback(new Error('invalid request object: missing appengine extensions'));
    return false;
  }
  return true;
};
...
AppEngine.prototype.systemGetBackgroundRequest_ = function(req, callback) {
  ...
  if (goog.isDefAndNotNull(req.appengine.moduleInstance)) {
    var moduleInstance = req.appengine.moduleInstance;
