...
AppEngine.prototype.logOneLine = function(req, message, logLevelOrCallback, maybeCallback) {
  ...
  if (typeof(logLevelOrCallback) === 'function') {
    ...
    // There is no well-defined callback to call, so throw an exception.
    throw new Error('incorrect arguments passed to logOneLine');
  }
  this.callApi_('logservice', 'Flush', req, makeLogFlushProto(message,
    this.getCurrentTime_(), level, this.serializer_), function(err) {
      callback(err);
    });
};
 ...
 */
function makeLogFlushProto(message, time, level, serializer) {
