...
AppEngine.prototype.logOneLine = function(req, message, callback) {
  this.callApi_('logservice', 'Flush', req, makeLogFlushProto(message,
    this.getCurrentTime_(), this.serializer_), function(err, apiResponse) {
      callback(err);
    });
};
...
AppEngine.prototype.middlewareBase_ = function(req, res, next) {
  ...
  var moduleName = this.getProcessEnv_('GAE_MODULE_NAME');
  if (goog.isDefAndNotNull(moduleName)) {
    info.moduleName = moduleName;
  }
  var moduleVersion = this.getProcessEnv_('GAE_MODULE_VERSION');
